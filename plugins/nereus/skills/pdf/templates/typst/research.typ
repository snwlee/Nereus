// 리서치 리포트 템플릿. 사용: #import "research.typ": doc / #show: doc.with(title: "...", font: "...")
#let doc(title: "문서", font: "Noto Sans KR", subtitle: none, author: none, date: datetime.today().display("[year]-[month]-[day]"), body) = {
  ...
  set page(paper: "a4", margin: (x: 22mm, y: 24mm), numbering: "1", number-align: center,
    header: context { if counter(page).get().first() > 1 { text(size: 9pt, fill: luma(110))[#title] ; h(1fr); text(size: 9pt, fill: luma(110))[RESEARCH REPORT] } })
  ...
  show heading.where(level: 1): it => { v(1.4em); text(size: 18pt, weight: 700, fill: rgb("#1b6b3a"))[#it.body]; v(0.4em); line(length: 100%, stroke: 0.6pt + rgb("#1b6b3a")); v(0.6em) }
  ...
  // 표지
  v(18%)
  text(size: 11pt, fill: rgb("#1b6b3a"), tracking: 0.12em, weight: 600)[RESEARCH REPORT]
  v(0.6em)
  text(size: 28pt, weight: 700)[#title]
  if subtitle != none { v(0.5em); text(size: 14pt, fill: luma(90))[#subtitle] }
  v(1.2em)
  line(length: 30%, stroke: 1.2pt + rgb("#1b6b3a"))
  v(0.8em)
  text(size: 10.5pt, fill: luma(110))[#if author != none [#author · ] #date]
  pagebreak()
  text(size: 9.5pt, fill: luma(110))[모든 주장에는 출처와 날짜가 있어야 한다. 추측은 추측이라고 표시한다.]; v(0.8em)
  body
}
