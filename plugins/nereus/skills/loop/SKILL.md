...
## 종료 코드
- `converged`: 전부 체크 + 프로젝트 테스트 러너 통과. nereus:review 로 넘어간다.
...
- **게이트 명령**: `--gate "<cmd>"` (종료코드 0만 통과). 없으면 수렴 판정과 같은 `run-tests.mjs`(프로젝트 러너 자동 감지)를 쓴다.
...
환경마다 명령이 다른 모양으로 나간다. 예컨대 rtk 훅이 깔린 환경에서는 `git status` 가
`rtk git status` 로 재작성돼 기본 목록과 어긋나고, 서브세션이 저장소 상태를 못 본다.
그런 프록시는 환경 고유라 배포 기본값에 넣지 않는다 — `.nereus/config.json` 에서 더한다:

```json
{ "loop": { "extraAllowedTools": ["Bash(rtk:*)"] } }
```

더하는 것으로 `git push` 나 권한 상승을 들여올 수는 없다. 그런 항목은 걸러진다.
