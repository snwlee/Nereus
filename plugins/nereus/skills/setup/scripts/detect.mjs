...
export const TOOLS = [
  ...
  { bin: "agy", label: "Antigravity CLI (Gemini 대체 채널)", required: false, group: "선택", install: { darwin: "brew install --cask antigravity-cli", win32: "irm https://antigravity.google/cli/install.ps1 | iex", linux: "curl -fsSL https://antigravity.google/cli/install.sh | bash" }, note: "설치 후 터미널에서 `agy` 한 번 실행해 Google 로그인" },
  { bin: "python3", label: "python3 (Gemini 웹세션 — 2차 의견·디자인 비평·이미지)", required: true, group: "코어", install: { darwin: "brew install python@3.12", win32: "winget install Python.Python.3.12", linux: "sudo apt-get install -y python3" }, note: "설치 후 Gemini 웹 로그인 쿠키가 필요하다 — skills/image/scripts/cookies-import.mjs" },
  ...
  { bin: "skillevaluator", label: "SkillEvaluator (스킬 품질 검사)", required: false, group: "선택", install: { darwin: "uv tool install git+https://github.com/NVIDIA/SkillEvaluator.git", win32: "uv tool install git+https://github.com/NVIDIA/SkillEvaluator.git", linux: "uv tool install git+https://github.com/NVIDIA/SkillEvaluator.git" } },
  ...
  { bin: "agent-reach", label: "Agent Reach (조사 — 플랫폼 원문 읽기)", required: false, group: "선택", install: { darwin: "uv tool install https://github.com/Panniantong/agent-reach/archive/main.zip", win32: "uv tool install https://github.com/Panniantong/agent-reach/archive/main.zip", linux: "uv tool install https://github.com/Panniantong/agent-reach/archive/main.zip" }, note: "**PyPI 의 `agent-reach` 는 동명이인이다**(jgalea/agent-reach, 채널 rss·youtube 둘뿐). 반드시 Panniantong 저장소에서 받는다. 설치 뒤 `agent-reach doctor` 로 살아 있는 채널을 본다. 기본은 읽기 전용 — `install --system` 은 기계를 바꾸므로 사용자 승인 뒤에만" },
