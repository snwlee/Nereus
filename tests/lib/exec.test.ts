...
describe("exec", () => {
  it("resolves bare command on posix via PATH", () => {
    ...
    expect(resolveExecutable("git", { platform: "darwin", env: { PATH: "/usr/local/bin:/usr/bin" }, exists })).toBe("/usr/bin/git");
    expect(resolveExecutable("nope", { platform: "darwin", env: { PATH: "/usr/bin" }, exists })).toBeNull();
  });
  ...
  it("which returns boolean-ish presence", () => {
    expect(which("git", { platform: "darwin", env: { PATH: "/usr/bin" }, exists: (p: string) => p === "/usr/bin/git" })).toBe("/usr/bin/git");
  });
});
