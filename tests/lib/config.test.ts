...
describe("config", () => {
  it("has sane defaults", () => {
    ...
    expect(DEFAULTS.baton).toEqual({ warn: 0.65, hard: 0.8 });
  ...
  it("loadConfig reads JSON files via injected reader and ignores missing or invalid ones", () => {
    ...
    expect(cfg.baton).toEqual({ warn: 0.65, hard: 0.9 });
  });
});
