...
import { checkFonts } from "../../plugins/nereus-game/lib/font-check.mjs";
import { parseCliInput } from "../../plugins/nereus-game/lib/cli-input.mjs";
...
describe("font check", () => {
  it("기준을 전부 만족하면 위반이 없다", () => {
    const r = checkFonts({ profile, fonts: [ok], locales, targetLocales: ["en", "ko"] });
  ...
  it("게임 임베딩이 허용되지 않은 폰트는 license-embedding", () => {
    ...
    const r = checkFonts({ profile, fonts, locales, targetLocales: ["en"] });
  ...
  it("임베딩 선언 자체가 없어도 license-embedding — 모름은 허용이 아니다", () => {
    ...
    expect(codes(checkFonts({ profile, fonts: [noDecl], locales, targetLocales: ["en"] }))).toContain("license-embedding");
  });

  it("로케일이 요구하는 스크립트를 아무 폰트도 안 덮으면 script-uncovered", () => {
    const fonts = [{ ...ok, scripts: ["latin"] }];
    const r = checkFonts({ profile, fonts, locales, targetLocales: ["en", "ko"] });
  ...
  it("여러 폰트가 나눠 덮으면 커버된 것으로 본다", () => {
    ...
    expect(codes(checkFonts({ profile, fonts, locales, targetLocales: ["en", "ko"] }))).not.toContain("script-uncovered");
  });

  it("유저 생성 텍스트가 있는데 서브셋하면 subset-unsafe — 닉네임이 두부가 된다", () => {
    const fonts = [{ ...ok, subset: true }];
    const r = checkFonts({ profile, fonts, locales, targetLocales: ["en"], userGeneratedText: true });
  ...
  it("유저 생성 텍스트가 없으면 서브셋은 정상이다", () => {
    const fonts = [{ ...ok, subset: true }];
    expect(codes(checkFonts({ profile, fonts, locales, targetLocales: ["en"], userGeneratedText: false }))).not.toContain("subset-unsafe");
  });
  ...
  it("최소 표시 크기 미만이면 min-size", () => {
    ...
    expect(codes(checkFonts({ profile, fonts, locales, targetLocales: ["en"] }))).toContain("min-size");
  });
