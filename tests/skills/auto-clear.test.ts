...
describe("auto-clear", () => {
  ...
  it("재개 프롬프트는 짧은 자연어이고 슬래시로 시작하지 않는다", () => {
    ...
    // 스킬 발동은 프롬프트에 지시문을 박아 넣는 대신 라우터가 보장한다(router.test.ts 가 교차 검증).
    expect(DEFAULT_RESUME_PROMPT.length).toBeLessThanOrEqual(20);
  });
