...
#### Scenario: 이름 충돌 없음
- **WHEN** 세 플러그인의 이름을 **종류별로**(스킬끼리·에이전트끼리) 모은다
- **THEN** 각 종류 안에 중복이 없다 — 한 플러그인에서 스킬과 에이전트가 같은 이름을 쓰는 것은
  이름 공간이 달라 정상이다(`nereus:seo`). 실제 사고는 두 플러그인의 에이전트끼리 겹친 것이었다

#### Scenario: 다른 플러그인을 import 하지 않는다
- **WHEN** `nereus-ads` 의 모든 소스에서 **import 지정자**를 뽑는다
- **THEN** `nereus-game` 이나 코어 내부 모듈을 가리키는 지정자가 없다 — 주석에 다른 플러그인
  이름이 나오는 것은 정상이다. 규율의 출처를 적는 것까지 막으면 이유가 사라진다
...
### Requirement: 라우트는 도메인 대표어 단독으로 잡지 않아야 한다
<!-- id: adsPlugin.narrowRoutes -->
<!-- entities: Plugin, Route -->
<!-- enforced: tests/smoke/ads-wiring.test.ts -->

모든 스킬은 라우트를 가져야 하고, 라우트는 `광고` 같은 도메인 대표어 단독으로 잡아서는 안 된다.
코어 라우터는 한 프롬프트당 스킬 2개(`MAX_HITS`)만 지목한다. 플러그인이 셋이면 넓은 라우트
하나가 다른 둘의 자리를 뺏고, `nereus-game:compliance`(규정)와 `ad-video` 델리게이트
(광고 영상)가 같은 단어를 놓고 실제로 경쟁한다.

#### Scenario: 모든 스킬이 도달 가능하다
- **WHEN** 플러그인의 스킬 디렉터리를 확장 선언의 라우트와 대조한다
- **THEN** 라우트가 없는 스킬이 없다 — 라우트 없는 스킬은 존재하지만 도달할 수 없다

#### Scenario: 대표어 단독을 잡지 않는다
- **WHEN** 각 라우트의 정규식에 `광고` 한 단어만 넣는다
