...
- [ ] T1. OCR 을 커밋된 브랜치 범위로 호출한다 [wave:1]
  ...
  - Steps:
    - [ ] 실패 테스트 작성:
      ...
      ```
    - [ ] 실패 확인: Run `npx vitest run tests/skills/review-merge.test.ts` · Expected: FAIL (ocrDelegateArgs 없음)
    - [ ] 최소 구현: `export function ocrDelegateArgs(base)` 가 문자열을 trim 해 비어 있지 않으면 `["--from", base, "--to", "HEAD"]`, 아니면 `[]` 를 돌려준다. `review/SKILL.md` 의 OCR 호출 설명을 `ocr delegate preview --from BASE --to HEAD` 형태로(BASE 는 비교 기준 브랜치) 고치고, 인자 없이 부르면 워크스페이스 모드로 떨어져 커밋된 변경이 빠진다는 사실을 적는다
    - [ ] 통과 확인: Run `npx vitest run tests/skills/review-merge.test.ts` · Expected: PASS
    - [ ] 커밋: `git add plugins/nereus/skills/review tests/skills/review-merge.test.ts && git commit -m "fix(harness): OCR 을 커밋된 브랜치 범위로 호출"`
...
- [ ] T2. 지배 전략 판정 [wave:1]
  ...
  - Steps:
    - [ ] 실패 테스트 작성:
      ...
      ```
    - [ ] 실패 확인: Run `npx vitest run tests/lib/balance-sim.test.ts` · Expected: FAIL (battle-pvp 프로파일·dominant 없음)
    - [ ] 최소 구현: `battle-pvp.json` 은 loop 를 조우·교전·정산·재장비, `balance.failureMode` 를 `dominant-strategy`, `balance.dominanceRatio` 를 3 으로 둔다. `narrative.json` 은 loop 를 도입·선택·전개·귀결, `failureMode` 를 `pacing`, `dominanceRatio` 를 3 으로 둔다. 두 프로파일 모두 `genre`·`metrics`·`cliffRatio` 를 채운다. `summarize` 가 `economy.options` 에서 효율(effect/cost)을 구해 최고 효율이 중앙값의 `dominanceRatio` 배를 넘으면 그 이름을 `dominant` 에 담는다. 선택지가 없거나 둘 이하면 빈 배열
    - [ ] 통과 확인: Run `npx vitest run tests/lib/balance-sim.test.ts tests/smoke/game-domain.test.ts` · Expected: PASS
    - [ ] 커밋: `git add plugins/nereus-game/lib/balance-sim.mjs plugins/nereus-game/profiles tests/lib/balance-sim.test.ts && git commit -m "feat(game): 지배 전략 판정과 프로파일 2종"`
...
- [ ] T3. 로블록스 2단 게이트 배선
  ...
  - Steps:
    - [ ] 실패 테스트 작성:
      ...
      ```
    - [ ] 실패 확인: Run `npx vitest run tests/lib/roblox-gate.test.ts` · Expected: FAIL (roblox-gate.mjs 없음)
    - [ ] 최소 구현: `robloxStageTwo` 가 `isRobloxProject` 로 대상을 가리고, `env` 에서 `ROBLOX_API_KEY`·`ROBLOX_UNIVERSE_ID`·`ROBLOX_PLACE_ID` 를 읽어 하나라도 없으면 `{ status: "unconfigured", pass: true }` 를 돌려주며 `run` 을 부르지 않는다. 전부 있으면 `deps.run`(기본값은 `runLuauTask`)을 불러 `pass` 로 `passed`/`failed` 를 정한다. `roblox/SKILL.md` 에 finish 단계에서 이 함수가 2단을 시도한다는 것과 세 환경변수 이름을 적는다
    - [ ] 통과 확인: Run `npx vitest run tests/lib/roblox-gate.test.ts` · Expected: PASS
    - [ ] 커밋: `git add plugins/nereus-game/lib/roblox-gate.mjs plugins/nereus-game/skills/roblox tests/lib/roblox-gate.test.ts && git commit -m "feat(game): 로블록스 2단 게이트 배선"`
...
- [ ] T4. 에셋 파이프라인 doctor
  ...
  - Steps:
    - [ ] 실패 테스트 작성:
      ...
      ```
    - [ ] 실패 확인: Run `npx vitest run tests/lib/asset-doctor.test.ts` · Expected: FAIL (asset-doctor.mjs 없음)
    - [ ] 최소 구현: 단계 `3d`(전제 `blender`), `2d`(전제 `COMFYUI_URL` 환경변수), `audio`(전제 `ELEVENLABS_API_KEY`) 를 각각 확인해 `{ stage, ok, why }` 배열을 돌려준다. `deps` 가 비어 있어도 기본값으로 동작하고 예외를 던지지 않는다. `asset/SKILL.md` 에 doctor 로 먼저 확인하라는 절차와 각 전제 이름을 적는다
    - [ ] 통과 확인: Run `npx vitest run tests/lib/asset-doctor.test.ts` · Expected: PASS
    - [ ] 커밋: `git add plugins/nereus-game/lib/asset-doctor.mjs plugins/nereus-game/skills/asset tests/lib/asset-doctor.test.ts && git commit -m "feat(game): 에셋 파이프라인 doctor"`
  - Done when: 네 테스트가 통과하고 도구 부재가 예외가 아니라 보고로 나온다

- [ ] T5. 배선 검증과 문서 정정 [flow]
  ...
  - Steps:
    - [ ] 실패 테스트 작성:
      ...
      ```
    - [ ] 실패 확인: Run `npx vitest run tests/smoke/game-domain.test.ts` · Expected: FAIL (README 미갱신)
    - [ ] 최소 구현: README 에 2단 게이트(`robloxStageTwo` 와 세 환경변수)와 에셋 doctor(`assetDoctor` 와 세 전제)를 안내하는 절을 추가하고, 장르 프로파일 표에 `battle-pvp`·`narrative` 행을 넣는다
    - [ ] 통과 확인: Run `npx vitest run tests/smoke/` · Expected: PASS
    - [ ] 커밋: `git add plugins/nereus-game/README.md tests/smoke/game-domain.test.ts && git commit -m "docs(game): 2단 게이트·doctor·프로파일 4종 안내"`
