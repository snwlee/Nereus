...
- [x] T1. 플러그인 뼈대와 정책 데이터
  ...
  - Steps:
    - [x] 실패 테스트를 `tests/lib/ad-policy-check.test.ts` 에 쓴다:
      ```ts
      ...
      ```
    - [x] 실패 확인: Run `npx vitest run tests/lib/ad-policy-check.test.ts` · Expected: FAIL (모듈 없음)
    - [x] `plugin.json` 을 쓴다: name `nereus-ads`, version `0.1.0`, license `MIT`.
    - [x] `ads-policy.json` 을 쓴다. 데모 단위는 WallpaperEngineApp `ad_unit_id_policy.dart` 에서 뜬다.
    ...
    - [x] 마켓플레이스에 `nereus-ads` 항목을 더한다(`source: ./plugins/nereus-ads`, version 일치).
    - [x] 통과 확인: Run `npx vitest run tests/lib/ad-policy-check.test.ts` · Expected: PASS
    - [x] 커밋: `git add plugins/nereus-ads .claude-plugin/marketplace.json tests/lib/ad-policy-check.test.ts && git commit -m "feat(ads): 플러그인 뼈대와 정책 데이터"`
  - Done when: `claude plugin validate plugins/nereus-ads` 가 통과하고 위 두 테스트가 초록이다

- [x] T2. 무효 트래픽 방어 — 데모/프로덕션 단위 판정
  ...
  - Steps:
    - [x] 실패 테스트를 덧붙인다:
      ...
      ```
    - [x] 실패 확인: Run `npx vitest run tests/lib/ad-policy-check.test.ts` · Expected: FAIL
    - [x] `checkAdPolicy` 를 쓴다. 데모 단위 판정은 `ads-policy.json` 의 표로만 한다 — 코드에 ID 를 박지 않는다.
    - [x] 통과 확인: Run `npx vitest run tests/lib/ad-policy-check.test.ts` · Expected: PASS
    - [x] 커밋: `git add plugins/nereus-ads/lib tests/lib/ad-policy-check.test.ts && git commit -m "feat(ads): 무효 트래픽 방어 — 디버그/릴리스 광고 단위 판정"`
...
- [x] T3. 첫 세션 보호와 동의 순서
  ...
  - Steps:
    - [x] 실패 테스트를 덧붙인다:
      ...
      ```
    - [x] 실패 확인: Run `npx vitest run tests/lib/ad-policy-check.test.ts` · Expected: FAIL
    - [x] 구현한다. 중단형 포맷 목록은 `ads-policy.json` 의 데이터다.
    - [x] 통과 확인: Run `npx vitest run tests/lib/ad-policy-check.test.ts` · Expected: PASS
    - [x] 커밋: `git add plugins/nereus-ads/lib tests/lib/ad-policy-check.test.ts && git commit -m "feat(ads): 첫 세션 보호와 동의 순서 게이트"`
...
- [x] T4. 타게팅 신호 일관성
  ...
  - Steps:
    - [x] 실패 테스트를 덧붙인다:
      ...
      ```
    - [x] 실패 확인: Run `npx vitest run tests/lib/ad-policy-check.test.ts` · Expected: FAIL
    - [x] 구현한다. 개인화 설정은 판정 대상이 아니다 — UMP 와 SDK 가 정한다.
    - [x] 통과 확인: Run `npx vitest run tests/lib/ad-policy-check.test.ts` · Expected: PASS
    - [x] 커밋: `git add plugins/nereus-ads/lib tests/lib/ad-policy-check.test.ts && git commit -m "feat(ads): 타게팅 신호 일관성 검사"`
  - Done when: 세 시나리오가 통과한다

- [x] T5. 퍼널 조언자 — 병목·믹스 역전·답할 수 없는 것
  ...
  - Steps:
    - [x] 실패 테스트를 쓴다:
      ...
      ```
    - [x] 실패 확인: Run `npx vitest run tests/lib/ad-funnel.test.ts` · Expected: FAIL
    - [x] 구현한다. 게이트가 아니므로 `violations` 를 내지 않는다.
    - [x] 통과 확인: Run `npx vitest run tests/lib/ad-funnel.test.ts` · Expected: PASS
    - [x] 커밋: `git add plugins/nereus-ads/lib tests/lib/ad-funnel.test.ts && git commit -m "feat(ads): 퍼널 병목 조언자"`
  - Done when: 네 시나리오가 통과한다

- [x] T6. 믹스 역전과 답할 수 없는 질문
  ...
  - Steps:
    - [x] 실패 테스트를 덧붙인다:
      ...
      ```
    - [x] 실패 확인: Run `npx vitest run tests/lib/ad-funnel.test.ts` · Expected: FAIL
    - [x] 구현한다. 역전 판정은 점유율 비교이지 절대값 비교가 아니다.
    - [x] 통과 확인: Run `npx vitest run tests/lib/ad-funnel.test.ts` · Expected: PASS
    - [x] 커밋: `git add plugins/nereus-ads/lib tests/lib/ad-funnel.test.ts && git commit -m "feat(ads): 믹스 역전과 답할 수 없는 질문"`
  - Done when: 세 시나리오가 통과한다
...
- [x] T7. 스킬 3종과 에이전트
  ...
  - Interfaces: 없음
  - Steps:
    - [x] `adpolicy` — 무효 트래픽·첫 세션·동의. `ad-policy-check.mjs` 호출법을 적는다.
          "정책 안전 처방은 노출을 줄이지 않는다"를 원칙으로 적는다.
    - [x] `adplacement` — 포맷별 배치. 전면 광고는 손가락이 멈춘 뒤에 띄우되
          **노출은 줄이지 않는다**(언제만 늦추고 최대 대기 후 통과). 쿨다운·세션 상한·이탈 지면.
    - [x] `adrevenue` — 레버 순서: show rate → 믹스 역전 → 고RPM 편중 → 집중도 → 트래픽 방어.
          `ad-funnel.mjs` 호출법과 "답할 수 없는 것"을 적는다. **실측 수치는 적지 않는다.**
    - [x] `agents/ads-engineer.md` — 이름이 다른 두 플러그인과 겹치지 않게 한다.
          TDD 절차는 `nereus:build` 것을 쓴다고 적는다.
    - [x] README 에 세 축과 두 검사기, 그리고 원격 설정 키 분류는 `nereus-game:liveops` 가 한다고 적는다.
    - [x] 통과 확인: Run `npx vitest run tests/smoke/ads-wiring.test.ts` · Expected: FAIL (아직 T8 미작성이면 건너뛴다)
    - [x] 커밋: `git add plugins/nereus-ads && git commit -m "docs(ads): 스킬 3종·에이전트·README"`
...
- [x] T8. 배선 가드와 프로세스 리그
  ...
  - Steps:
    - [x] 실패 테스트 `tests/smoke/ads-wiring.test.ts` 를 쓴다:
      ```ts
      ...
      ```
    - [x] 실패 테스트 `tests/smoke/ads-rig.test.ts` 를 쓴다:
      ```ts
      ...
      ```
    - [x] 실패 확인: Run `npx vitest run tests/smoke/ads-wiring.test.ts tests/smoke/ads-rig.test.ts` · Expected: FAIL
    - [x] `nereus-extension.json` 에 라우트 3개를 쓴다. **좁게** 쓴다 — `광고` 단독 단어를 쓰지 않는다
          (`nereus-game:compliance` 와 경쟁한다). 포맷 이름·AdMob 어휘·지표 이름으로 잡는다.
    - [x] 두 lib 에 프로세스 진입점을 붙인다. `pathToFileURL` 로 비교한다. `process.exit(0)` 금지.
    - [x] 통과 확인: Run `npx vitest run tests/smoke/ads-wiring.test.ts tests/smoke/ads-rig.test.ts` · Expected: PASS
    - [x] 역검증: 라우트 하나를 지우면 FAIL 하는지 확인하고 되돌린다
    - [x] 커밋: `git add plugins/nereus-ads tests/smoke && git commit -m "feat(ads): 라우트 배선과 프로세스 리그"`
  - Done when: 가드가 실제로 물고 역검증이 통과한다

- [x] T9. 설치·충돌 점검 · 전체 테스트 · 마무리
  - Files: none
  - Interfaces: 없음
  - Steps:
    - [x] Run `claude plugin validate plugins/nereus-ads` · Expected: 통과
    - [x] Run `claude plugin marketplace update nereus && claude plugin install nereus-ads@nereus` · Expected: 설치 성공
